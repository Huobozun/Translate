...
if __name__ == "__main__":
    ...
    for i in range(0,len(Filelist)):
        ...
        fw=get_content('resultall3/firstword/first-',Filelist[i])#list里类型为list
        fword=get_word('resultall2/',Filelist[i])#list里类型为str
        fm=get_content('resultall2/Mark_word1/M10-',Filelist[i])
        with open('/home/zjg/code2/resultall3.2/'+Filelist[i],'w',newline='')as fa:
            for i1 in range(0,len(ft)):
                ...
                if(fm1[0]==0):#如果第一个词条没翻译，需要加上第一个词条的翻译结果
                    if(len(fword1)==0):
                        fword2=fw1[0]+'###'+ft1[0]+'###'+fr1[0]
                    else:
                        fword2=fw1[0]+'###'+ft1[0]+'###'+fr1[0]+' [SEP] '+fword1
                else:
                    fword2=fword1
                

                fa.write('%s\t%s\n' %(ftitle[i1],fword2))#重新写进新的文件
            fa.close()
