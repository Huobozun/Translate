...
if __name__ == "__main__":
    ...
    for i in range(0,len(Filelist)):
        ...
        fm=get_index('resultall2/Mark_word1/M10-',Filelist[i])#list里类型为list
        with open('/home/zjg/code2/resultall3/Mark_word2/M10and1-'+Filelist[i],'w',newline='')as fa:
            for i1 in range(0,len(fm)):
                fm1=fm[i1]
                x=0
                for i11 in range(0,len(fm1)):
                    if(fm1[i11]>0):
                        x=1#如果一行中已经有词条被翻译过，就标记x=1
                if(x==0):#如果x=0说明词条中没有词被翻译过，则翻译第一个词，则把第一个位置标记为1
                    fm1[0]=1
