...
def file_fre(file,ii):
     #读入文件频率
    data=[]
    with open('/home/zjg/code3.31/fre/fre2.0/'+file.replace('.tsv','')+'/fre0-Sam1-pubmed_8_years_'+str(ii)+file,'r',encoding='utf-8')as fii:
        for lines in fii:
            json_data=lines
...
if __name__ == "__main__":
    ...
    #for i in range(0,1):
        for ii in range(0,18):#读取每一个文件依次累加
            if(ii==0):
                a=file_fre(Filelist[i],ii)
            else:
                a=add_fre(a,Filelist[i],ii)
        #加完之后写入总的文件
        with open('/home/zjg/code3.31/fre/fre2.0/'+'All-'+Filelist[i],'w',newline='')as fw:
